...
const menuGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'subscription', label: 'Subscription', icon: Package }, // Added Subscription menu button
    ]
  },
  {
...
export default function Sidebar({ currentPage, onNavigate, isOpen, onToggle }: SidebarProps) {
  ...
  const showSidebar = isOpen || mobileOpen;
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 w-full bg-slate-900 text-white flex items-center justify-between p-4 z-40">
        <div className="font-bold text-lg">MediCore</div>
        <button
          aria-label={mobileOpen ? "Close sidebar" : "Open sidebar"}
          onClick={() => setMobileOpen(!mobileOpen)}
          className="p-2 rounded-lg focus:outline-none hover:bg-slate-800"
        >
          {mobileOpen ? <Cross className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
        </button>
      </div>

      {/* Overlay for mobile when sidebar is open */}
      {mobileOpen && (
        <div className="fixed inset-0 bg-black/40 z-30 lg:hidden" onClick={() => setMobileOpen(false)} aria-label="Close sidebar overlay" />
      )}

      <aside className={`
        fixed lg:relative z-40 h-screen flex flex-col bg-slate-900 text-white transition-all duration-300
        ${mobileOpen ? 'w-64' : 'w-0'}
      ...
      `} aria-label="Sidebar navigation">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-700">
          ...
          <button onClick={onToggle} className="p-1.5 rounded-lg hover:bg-slate-700 transition-colors hidden lg:block">
